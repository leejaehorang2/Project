...
namespace NepesInterface
{
	...
	//Lot Cycle
	//Lot Card와 사번 Loading-> 첫 번째 그랩 -> Lot Number와 사번 리딩-> GetRecipeTypeFromMES()-> Recipe Load 되면 -> GetReelInformationFromMES()

		//Reel Cycle Start(Lot에 있는 Reel 개수만큼 반복)
		//ROI에 맞춰서 제품 로딩 후 첫 번째 그랩
		
			//Label Cycle(Reel에 있는 Label 개수만큼 반복)
			//Label Localization -> Barcode Reading -> SetImagePath() -> StartTextDetection() -> SetBarcodeReadingResult() -> AlignLabelDataByMESInformation()
			//-> StartTextClassification() -> CompareResultToMES() -> GetResultDataSize() -> GetResultData() -> WriteComtestData()-> ClearLabelData() -> Label Localization 스텝으로 이동해서 라벨 개수만큼 반복
		
			//Label Cycle End
			//모든 라벨 검사 끝났으면 -> ROI에 맞춰서 제품 로딩 스텝으로 이동
		
		//Reel Cycle End
		//ClearLotData() Lot Card와 사번 Loading 스텝으로 복귀
